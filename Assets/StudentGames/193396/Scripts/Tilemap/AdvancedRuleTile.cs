...
namespace _193396
{
	...
	public class AdvancedRuleTile : TileBase
	{
		...
		/// <summary>
		/// Tiles that this Rule Tile can connect with.
		/// </summary>
		public TileBase[] m_ConnectableTiles = Array.Empty<TileBase>();

		/// <summary>
		/// Angle in which the AdvancedRuleTile is rotated by for matching in Degrees.
		/// </summary>
		public virtual int m_RotationAngle => 90;

		/// <summary>
		/// Number of rotations the AdvancedRuleTile can be rotated by for matching.
		/// </summary>
		...
		public class TilingRuleOutput
		{
			...
			/// <summary>
			/// The weights of individual output Sprites for this Rule.
			/// </summary>
			public int[] m_Weights = new int[1];
			/// <summary>
			...
			public class Neighbor
			{
				...
				/// <summary>
				/// The Rule Tile will check if the contents of the cell in that direction is not an instance of any Tile.
				/// If it is, the rule will fail.
				/// </summary>
		...
		public class TilingRule : TilingRuleOutput
		{
			...
			/// <summary>
			/// * Preset this list to AdvancedRuleTile backward compatible, but not support for HexagonalRuleTile backward compatible.
			/// </summary>
		...
		/// </summary>
		[HideInInspector] public List<TilingRule> m_TilingRules = new List<AdvancedRuleTile.TilingRule>();

		/// <summary>
		/// Returns a set of neighboring positions for this AdvancedRuleTile
		/// </summary>
		...
		/// <summary>
		/// Updates the neighboring positions of this AdvancedRuleTile
		/// </summary>
				...
				if (RuleMatches(rule, position, tilemap, ref transform))
				{
					switch (rule.m_Output)
					{
						...
						case TilingRuleOutput.OutputSprite.Random:
							...
							break;
						case TilingRuleOutput.OutputSprite.WeightedRandom:
							...
							var randomWeight = UnityEngine.Random.Range(0, rule.m_Weights.Aggregate((a, b) => a + b));
							for (int i = 0; i < rule.m_Weights.Length; i++)
								if ((randomWeight -= rule.m_Weights[i]) < 0)
								{
									tileData.sprite = rule.m_Sprites[i];
									break;
								}
		...
		static KeyValuePair<HashSet<TileBase>, HashSet<Vector3Int>> CachingTilemapNeighborPositions(Tilemap tilemap)
		{
			...
			for (int i = 0; i < usedTileCount; i++)
			{
				...
				usedTiles.Add(tile);
				AdvancedRuleTile ruleTile = null;

				if (tile is AdvancedRuleTile rt)
					ruleTile = rt;
		...
		/// <summary>
		/// Returns custom fields for this AdvancedRuleTile
		/// </summary>
		/// <param name="isOverrideInstance">Whether override fields are returned</param>
		/// <returns>Custom fields for this AdvancedRuleTile</returns>
		public FieldInfo[] GetCustomFields(bool isOverrideInstance)
		{
			return this.GetType().GetFields(BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic)
				.Where(field => typeof(AdvancedRuleTile).GetField(field.Name) == null)
		...
		public virtual bool RuleMatch(int neighbor, TileBase other)
		{
			...
			switch (neighbor)
			{
				case TilingRuleOutput.Neighbor.This: return other == this || m_ConnectableTiles.Contains(other);
				case TilingRuleOutput.Neighbor.NotThis: return other != this && !m_ConnectableTiles.Contains(other);
				case TilingRuleOutput.Neighbor.Nothing: return other == null;
			}
			return true;
		}
