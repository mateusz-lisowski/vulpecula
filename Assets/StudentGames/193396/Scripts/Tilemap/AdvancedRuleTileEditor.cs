...
namespace _193396
{
	...
	/// </summary>
	[CustomEditor(typeof(AdvancedRuleTile), true)]
	...
	public class AdvancedRuleTileEditor : Editor
	{
		...
		private const string s_XDarkIconString = "";
		...
		private static readonly string k_UndoName = L10n.Tr("Change AdvancedRuleTile");
		...
		public static Texture2D[] arrows
		{
			get
			{
				if (s_Arrows == null || s_Arrows.Length != 11)
				{
					...
					s_Arrows[10] = Base64ToTexture(s_XDarkIconString);
				}
				return s_Arrows;
			}
		}
		...
		/// <summary>
		/// The AdvancedRuleTile being edited
		/// </summary>
		public AdvancedRuleTile tile => target as AdvancedRuleTile;
		...
		/// <summary>
		/// Whether the AdvancedRuleTile can extend its neighbors beyond directly adjacent ones
		/// </summary>
		...
		{
			m_ReorderableList = new ReorderableList(tile != null ? tile.m_TilingRules : null, typeof(AdvancedRuleTile.TilingRule), true, true, true, true);
		...
		public virtual BoundsInt GetRuleGUIBounds(BoundsInt bounds, AdvancedRuleTile.TilingRule rule)
		{
			if (extendNeighbor)
			{
				bounds.xMin--;
		...
		{
			AdvancedRuleTile.TilingRule rule = tile.m_TilingRules[index];
		...
		public float GetElementHeight(AdvancedRuleTile.TilingRule rule)
		{
			BoundsInt bounds = GetRuleGUIBounds(rule.GetBounds(), rule);

			float inspectorHeight = GetElementHeight(rule as AdvancedRuleTile.TilingRuleOutput);
		...
		public float GetElementHeight(AdvancedRuleTile.TilingRuleOutput rule)
		{
			float inspectorHeight = k_DefaultElementHeight + k_PaddingBetweenRules;

			switch (rule.m_Output)
			{
				case AdvancedRuleTile.TilingRuleOutput.OutputSprite.Random:
				case AdvancedRuleTile.TilingRuleOutput.OutputSprite.Animation:
					inspectorHeight = k_DefaultElementHeight + k_SingleLineHeight * (rule.m_Sprites.Length + 3) + k_PaddingBetweenRules;
					break;
				case AdvancedRuleTile.TilingRuleOutput.OutputSprite.WeightedRandom:
					inspectorHeight = k_DefaultElementHeight + k_SingleLineHeight * (rule.m_Sprites.Length + 1) + k_PaddingBetweenRules;
					break;
			}
		...
		/// <summary>
		/// Gets the GUI matrix size for a Rule of a AdvancedRuleTile
		/// </summary>
		/// <param name="bounds">Cell bounds of the Rule.</param>
		/// <returns>Returns the GUI matrix size for a Rule of a AdvancedRuleTile.</returns>
		...
		private void OnAddElement(object obj)
		{
			...
			AdvancedRuleTile.TilingRule rule = new AdvancedRuleTile.TilingRule();
			rule.m_Output = AdvancedRuleTile.TilingRuleOutput.OutputSprite.Single;
		...
		/// <summary>
		/// Saves any changes to the AdvancedRuleTile
		/// </summary>
		...
		/// <summary>
		/// Gets all RuleOverrideTiles which override the given AdvancedRuleTile
		/// </summary>
		/// <param name="target">AdvancedRuleTile which has been updated</param>
		/// <returns>A list of RuleOverrideTiles which override the given AdvancedRuleTile</returns>
		public static List<RuleOverrideTile> FindAffectedOverrideTiles(AdvancedRuleTile target)
		{
		...
		private void ResizeRuleTileList(int count)
		{
			...
			if (isEmpty)
			{
				for (int i = 0; i < count; ++i)
					tile.m_TilingRules[i] = new AdvancedRuleTile.TilingRule();
			}
			UpdateTilingRuleIds();
		}

		/// <summary>
		/// Draw editor fields for custom properties for the AdvancedRuleTile
		/// </summary>
		...
		/// <summary>
		/// Gets the index for a Rule with the AdvancedRuleTile to display an arrow.
		/// </summary>
		/// <param name="position">The relative position of the arrow from the center.</param>
		/// <returns>Returns the index for a Rule with the AdvancedRuleTile to display an arrow.</returns>
		...
		public virtual void RuleOnGUI(Rect rect, Vector3Int position, int neighbor)
		{
			switch (neighbor)
			{
				case AdvancedRuleTile.TilingRuleOutput.Neighbor.This:
					GUI.DrawTexture(rect, arrows[GetArrowIndex(position)]);
					break;
				case AdvancedRuleTile.TilingRuleOutput.Neighbor.NotThis:
					GUI.DrawTexture(rect, arrows[9]);
					break;
				case AdvancedRuleTile.TilingRuleOutput.Neighbor.Nothing:
		...
		public virtual void RuleTransformOnGUI(Rect rect, AdvancedRuleTile.TilingRuleOutput.Transform ruleTransform)
		{
			switch (ruleTransform)
			{
				case AdvancedRuleTile.TilingRuleOutput.Transform.Rotated:
					GUI.DrawTexture(rect, autoTransforms[0]);
					break;
				case AdvancedRuleTile.TilingRuleOutput.Transform.MirrorX:
					GUI.DrawTexture(rect, autoTransforms[1]);
					break;
				case AdvancedRuleTile.TilingRuleOutput.Transform.MirrorY:
					GUI.DrawTexture(rect, autoTransforms[2]);
					break;
				case AdvancedRuleTile.TilingRuleOutput.Transform.Fixed:
					GUI.DrawTexture(rect, autoTransforms[3]);
					break;
				case AdvancedRuleTile.TilingRuleOutput.Transform.MirrorXY:
		...
		public void RuleNeighborUpdate(Rect rect, AdvancedRuleTile.TilingRule tilingRule, Dictionary<Vector3Int, int> neighbors, Vector3Int position)
		{
		...
		public void RuleTransformUpdate(Rect rect, AdvancedRuleTile.TilingRule tilingRule)
		{
			if (Event.current.type == EventType.MouseDown && ContainsMousePosition(rect))
			{
				tilingRule.m_RuleTransform = (AdvancedRuleTile.TilingRuleOutput.Transform)(int)Mathf.Repeat((int)tilingRule.m_RuleTransform + GetMouseChange(), Enum.GetValues(typeof(AdvancedRuleTile.TilingRule.Transform)).Length);
				GUI.changed = true;
				Event.current.Use();
			}
		}
		...
		/// <summary>
		/// Draws a Rule Matrix for the given Rule for a AdvancedRuleTile.
		/// </summary>
		...
		public virtual void RuleMatrixOnGUI(AdvancedRuleTile tile, Rect rect, BoundsInt bounds, AdvancedRuleTile.TilingRule tilingRule)
		{
		...
		/// <summary>
		/// Draws a Rule Matrix Icon for the given matching Rule for a AdvancedRuleTile with the given position
		/// </summary>
		/// <param name="tilingRule">Tile to draw rule for.</param>
		/// <param name="neighbors">A dictionary of neighbors</param>
		/// <param name="position">The relative position of the neighbor matching Rule</param>
		/// <param name="rect">GUI Rect to draw icon at</param>
		public void RuleMatrixIconOnGUI(AdvancedRuleTile.TilingRule tilingRule, Dictionary<Vector3Int, int> neighbors, Vector3Int position, Rect rect)
		{
		...
		public virtual void SpriteOnGUI(Rect rect, AdvancedRuleTile.TilingRuleOutput tilingRule)
		{
		...
		public void RuleInspectorOnGUI(Rect rect, AdvancedRuleTile.TilingRuleOutput tilingRule)
		{
			float y = rect.yMin;
			...
			tilingRule.m_Output = (AdvancedRuleTile.TilingRuleOutput.OutputSprite)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_Output);
			y += k_SingleLineHeight;

			if (tilingRule.m_Output == AdvancedRuleTile.TilingRuleOutput.OutputSprite.Animation)
			{
			...
			if (tilingRule.m_Output == AdvancedRuleTile.TilingRuleOutput.OutputSprite.Random)
			{
				...
				tilingRule.m_RandomTransform = (AdvancedRuleTile.TilingRuleOutput.Transform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_RandomTransform);
				y += k_SingleLineHeight;
			}

			if (tilingRule.m_Output != AdvancedRuleTile.TilingRuleOutput.OutputSprite.Single)
			{
				bool isWeigthed = tilingRule.m_Output == AdvancedRuleTile.TilingRuleOutput.OutputSprite.WeightedRandom;

				GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight)
					, tilingRule.m_Output == AdvancedRuleTile.TilingRuleOutput.OutputSprite.Animation ? Styles.tilingRulesAnimationSize : Styles.tilingRulesRandomSize);
				EditorGUI.BeginChangeCheck();
				...
				if (EditorGUI.EndChangeCheck())
				{
					...
					if (isWeigthed)
						Array.Resize(ref tilingRule.m_Weights, Math.Max(newLength, 1));
				}
				y += k_SingleLineHeight;

				if (isWeigthed && tilingRule.m_Weights.Length != tilingRule.m_Sprites.Length)
					Array.Resize(ref tilingRule.m_Weights, tilingRule.m_Sprites.Length);

				for (int i = 0; i < tilingRule.m_Sprites.Length; i++)
				{
					if (isWeigthed)
						tilingRule.m_Weights[i] = Math.Max(EditorGUI.IntField(new Rect(rect.xMin + 42f, y, 32f, k_SingleLineHeight), tilingRule.m_Weights[i]), 1);
		...
		/// <summary>
		/// Whether the AdvancedRuleTile has a preview GUI
		/// </summary>
		...
		/// <summary>
		/// Draws the preview GUI for the AdvancedRuleTile
		/// </summary>
		...
		/// <summary>
		/// Creates a Preview for the AdvancedRuleTile.
		/// </summary>
		...
		/// <summary>
		/// Renders a static preview Texture2D for a AdvancedRuleTile asset
		/// </summary>
		/// <param name="assetPath">Asset path of the AdvancedRuleTile</param>
		...
		/// <returns>Texture2D containing static preview for the AdvancedRuleTile asset</returns>
		...
		class RuleTileRuleWrapper
		{
			...
			/// </summary>
			[SerializeField]
			public List<AdvancedRuleTile.TilingRule> rules = new List<AdvancedRuleTile.TilingRule>();
		}

		/// <summary>
		/// Copies all Rules from a AdvancedRuleTile to the clipboard
		/// </summary>
		/// <param name="item">MenuCommand storing the AdvancedRuleTile to copy from</param>
		[MenuItem("CONTEXT/AdvancedRuleTile/Copy All Rules")]
		public static void CopyAllRules(MenuCommand item)
		{
			AdvancedRuleTile tile = item.context as AdvancedRuleTile;
			if (tile == null)
				return;
		...
		/// <summary>
		/// Pastes all Rules from the clipboard to a AdvancedRuleTile
		/// </summary>
		/// <param name="item">MenuCommand storing the AdvancedRuleTile to paste to</param>
		[MenuItem("CONTEXT/AdvancedRuleTile/Paste Rules")]
